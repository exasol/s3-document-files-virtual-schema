import { ApiError, Context } from "@exasol/extension-manager-interface";
...
export function installExtension(context: Context, extension: ExtensionInfo, versionToInstall: string): void {
    if (extension.version !== versionToInstall) {
        throw new ApiError(400, `Installing version '${versionToInstall}' not supported, try '${extension.version}'.`)
    }
