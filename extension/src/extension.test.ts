...
describe("S3 VS Extension", () => {
    ...
    describe("install()", () => {
        it("executes expected statements", () => {
            ...
            const expectedComment = `Created by Extension Manager for ${EXTENSION_NAME} extension ${CONFIG.version}`
    ...
    describe("addInstance()", () => {
        describe("connection parameters converted", () => {
            ...
            for (const test of tests) {
                it(test.name, () => {
                    ...
                    expect(context.mocks.sqlExecute.mock.calls[0]).toEqual([`CREATE OR REPLACE CONNECTION "NEW_S3_VS_CONNECTION" TO '' USER '' IDENTIFIED BY '${test.expected}'`])
                })
            }
        })
        it("executes expected statements", () => {
            ...
            expect(calls[0]).toEqual([`CREATE OR REPLACE CONNECTION "NEW_S3_VS_CONNECTION" TO '' USER '' IDENTIFIED BY '{"awsAccessKeyId":"id"}'`])
        ...
        it("escapes single quotes", () => {
            ...
            const comment = `Created by Extension Manager for ${EXTENSION_NAME} vs''name`
            expect(calls[2]).toEqual([`COMMENT ON CONNECTION "vs'name_CONNECTION" IS '${comment}'`])
            expect(calls[3]).toEqual([`COMMENT ON SCHEMA "vs'name" IS '${comment}'`])
        })
    ...
    describe("findInstances()", () => {
        ...
        it("filters by schema and script name", () => {
            ...
            expect(queryCalls[0]).toEqual(["SELECT SCHEMA_NAME FROM SYS.EXA_ALL_VIRTUAL_SCHEMAS WHERE ADAPTER_SCRIPT = ?||'.'||?  ORDER BY SCHEMA_NAME", "ext-schema", "S3_FILES_ADAPTER"])
        })
    })
